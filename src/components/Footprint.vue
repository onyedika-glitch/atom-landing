<template>
  <div class="h-30"></div>
  <section class="bg-[#ffffff] py-12 sm:py-24 px-4 flex items-center justify-center px-4">
    <div class="h-10"></div>
      <div class="w-full max-w-5xl py-8 sm:py-16 flex flex-col items-center text-center">
      <div class="text-center font-figtree leading-[72px] tracking-[-0.02em] text-[56px]">
      <!-- Heading Section -->
      <h2 class="font-figtree font-normal text-3xl sm:text-5xl md:text-7xl leading-[1.2] tracking-[-0.02em] text-[#181D27] text-center">
        Our Customer Footprint
      </h2>

    <div class="h-10"></div>
      <!-- Line break and paragraph -->
      <p class="text-xl md:text-2xl text-gray-600 max-w-2xl mt-4">
        See how we're making an impact across regions and industries.
      </p>
        </div>
      <div class="h-10"></div>
      <!-- Space before stats -->
        <!-- Big Container -->
      <section class="w-full max-w-5xl h-[240px] gap-16 rounded-2xl p-28 bg-gray-100 mx-auto flex flex-col sm:flex-row items-center justify-between px-4">

      <div class="text-center flex-1">
            <div class="text-[#1AA9B9] text-4xl sm:text-5xl font-extrabold">1000+</div>
            <div class="text-[#111827] text-base sm:text-lg font-semibold mt-2">Clients</div>
          </div>

          <!-- Stat 2 -->
          <div class="text-center flex-1">
            <div class="text-[#1AA9B9] text-5xl font-extrabold">600%</div>
            <div class="text-[#111827] text-lg font-semibold mt-2">Value increase</div>
          </div>

          <!-- Stat 3 -->
          <div class="text-center flex-1">
            <div class="text-[#1AA9B9] text-5xl font-extrabold">830</div>
            <div class="text-[#111827] text-lg font-semibold mt-2">Finished products</div>
          </div>
        </section>



        </div>


  </section>
</template>
