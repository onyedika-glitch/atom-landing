<template>
  <div class="h-30"></div>
  <section class="w-full mx-auto w-[1380px] bg-[#ffffff] py-200 px-4 flex items-center justify-center">
    <div class="h-10"></div>
    <div class="w-full max-w-5xl py-[200px] flex flex-col items-center text-center">
      <div class="text-center font-figtree leading-[72px] tracking-[-0.02em] text-[56px]">
      <!-- Heading Section -->
      <h2 class="font-figtree font-normal text-8xl text-[40px] leading-[1.2] tracking-[-0.02em] text-[#181D27] text-center">
        Our Customer Footprint
      </h2>

    <div class="h-10"></div>
      <!-- Line break and paragraph -->
      <p class="text-xl md:text-2xl text-gray-600 max-w-2xl mt-4">
        See how we're making an impact across regions and industries.
      </p>
        </div>
      <div class="h-10"></div>
      <!-- Space before stats -->
        <!-- Big Container -->
      <section class="w-[1216px] h-[240px] gap-16 rounded-2xl p-28 bg-gray-100 mx-auto flex items-center justify-between">
        <!-- your content here -->

      <div class="text-center flex-1">
            <div class="text-[#1AA9B9] text-5xl font-extrabold">1000+</div>
            <div class="text-[#111827] text-lg font-semibold mt-2">Clients</div>
          </div>

          <!-- Stat 2 -->
          <div class="text-center flex-1">
            <div class="text-[#1AA9B9] text-5xl font-extrabold">600%</div>
            <div class="text-[#111827] text-lg font-semibold mt-2">Value increase</div>
          </div>

          <!-- Stat 3 -->
          <div class="text-center flex-1">
            <div class="text-[#1AA9B9] text-5xl font-extrabold">830</div>
            <div class="text-[#111827] text-lg font-semibold mt-2">Finished products</div>
          </div>
        </section>



        </div>


  </section>
</template>
